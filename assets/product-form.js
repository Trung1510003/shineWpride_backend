...
class ProductFormComponent extends Component {
  ...
  handleCheckoutClick(event) {
    ...
    if (this.#variantChangeInProgress) return;

    this.#processAddToCart(undefined, undefined, event, true);
  }
   ...
   * @param {Event} [event]
   * @param {boolean} [redirectToCheckout]
   */
  #processAddToCart(overrideVariantId, overrideQuantity, event, redirectToCheckout = false) {
  ...
  #onVariantUpdate = async (event) => {
    ...
    const checkoutButton = this.querySelector('[data-testid="product-checkout-button"]');
    if (checkoutButton instanceof HTMLButtonElement) {
      checkoutButton.disabled = event.detail.resource == null || event.detail.resource.available === false;
    }
